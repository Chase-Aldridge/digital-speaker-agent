...
export default function Dashboard() {
  ...
  const upcoming = subs
    ...
    .sort((a, b) =>
      a.opportunity!.deadline! > b.opportunity!.deadline! ? 1 : -1,
    );
  ...
  const steps = [
    {
      ...
      title: "Tune your speaker profile",
      sub: `${completeness}% complete`,
      advice:
        "Add your headline, topics, and speaking history, then let the agent write your bio for you.",
      to: "/app/profile",
      cta: "Tune profile",
    },
    {
      done: (stats?.total ?? 0) > 0,
      title: "Find and track your first gig",
      sub: "Pick one off the board",
      advice:
        "Hit Discover and the agent goes and finds events and podcasts that fit you. Track the ones you like.",
      to: "/app/opportunities",
      cta: "Find gigs",
    },
    {
      done: subs.some((s) => s.pitch),
      title: "Generate an application in your voice",
      sub: "One click in the booth",
      advice:
        "Open a tracked gig and generate a tailored application built from your profile and that event.",
      to: "/app/pipeline",
      cta: "Open log",
    },
  ];
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 border-b-[1.5px] rule pb-5">
        <div>
          ...
          <p className="label mt-2 text-ink-3">
            {onAir ? "You are on the air" : "Pre-flight in progress"} ·{" "}
            {todayStamp()}
          </p>
        </div>
          ...
          {onAir ? (
            <Card className="flex items-center gap-4 p-6">
              ...
              <div>
                ...
                <p className="text-sm text-ink-2">
                  Profile tuned, gigs tracked, applications rolling. Keep the
                  board warm.
                </p>
              </div>
            </Card>
            ...
            <Card className="p-6">
              ...
              <h2 className="mt-1 font-display text-2xl font-semibold text-ink">
                Go on air in 3 steps
              </h2>
              <p className="mt-1 text-sm text-ink-2">
                Do these in order. We walk you through each one.
              </p>
                ...
                {steps.map((s, i) => {
                  ...
                  return (
                    <div
                      key={s.title}
                      className={
                        "flex items-start gap-4 rounded-md border-[1.5px] px-4 py-3 " +
                        (active
                    ...
                    >
                      <span
                        className={
                          "mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md border-[1.5px] border-ink font-display font-semibold " +
                          (s.done
                            ? "bg-signal text-paper-2"
                            : active
                              ? "bg-paper-2 text-ink"
          ...
          {upcoming.length > 0 ? (
            <Card className="p-6">
              <div className="flex items-center justify-between border-b-[1.5px] rule pb-3">
                ...
                <Link
                  to="/app/pipeline"
                  className="label text-signal-2 hover:text-signal"
                >
                  Full log →
                </Link>
              </div>
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <p className="mt-1 text-sm text-ink-3">
              How complete your speaker profile is. A fuller profile writes
            ...
            <div className="label mt-2 text-ink-2">
              ...
              {STATUS_ORDER.map((st) => {
                ...
                return (
                  <div key={st} className="flex items-center gap-3">
                    <span className="label w-24 shrink-0 text-ink-3">
          ...
          <Card className="flex items-center gap-3 p-5">
            ...
            <div className="text-sm text-ink-2">
              Track a gig, then generate an application in your voice in one
              click.
            </div>
          </Card>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <Sparkles
          className={"h-5 w-5 " + (urgent ? "text-signal" : "text-ink-3")}
        />
        <div>
          <div className="label text-ink-3">Next deadline</div>
          <div className="text-sm font-semibold text-ink">
            {sub.opportunity?.organization}
          </div>
        </div>
      </div>
      <div className="text-right">
        ...
        <div className="label text-ink-3">
          {formatDate(sub.opportunity?.deadline)}
        </div>
      </div>
    </div>
  );
